    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            ...
            try:
                ...
                self.stdout.write(f'Error clearing warehouses: {e}')
            ...
            for warehouse in warehouses:
                ...
                for i in range(warehouse_sales_count):
                    try:
                        ...
                        warehouse_stock_movements = StockMovement.objects.filter(
                            warehouse=warehouse,
                            movement_type='in'
                        )
                        
                        if warehouse_stock_movements.exists():
                            # Select 1-3 products for this sale
                            selected_movements = random.sample(
                                list(warehouse_stock_movements), 
                                min(random.randint(1, 3), warehouse_stock_movements.count())
                            )
                            
                            total_amount = 0
                            for movement in selected_movements:
                                # Calculate sale amount (assuming we can get product from movement reference)
                                quantity_sold = random.randint(5, min(50, movement.quantity // 2))
                                # Use a base price calculation
            ...
            # Generate inter-warehouse transfers
            ...
            for i in range(15):
                try:
                    ...
                    # Get a product that exists in from_warehouse
                    from_movements = StockMovement.objects.filter(
                        warehouse=from_warehouse,
                        movement_type='in'
                    )
                    
                    if from_movements.exists():
                        movement = random.choice(from_movements)
                        transfer_qty = random.randint(5, min(50, movement.quantity // 3))
                        
                        # Create transfer record
                        transfer = InventoryTransfer.objects.create(
                            product=None,  # We'll use reference instead
                            quantity=transfer_qty,
                            from_location=from_warehouse.name,
                            to_location=to_warehouse.name,
                            requested_by=from_warehouse.manager,
