...
const ProcurementRequestDialog = ({ open, onClose, onSubmit, editData = null }) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      setError(err.message || 'Failed to submit procurement request');
    } finally {
      setLoading(false);
    }
  };
  ...
  const getCurrentStageIndex = () => {
    if (!editData || !editData.current_stage) return 0;
    return APPROVAL_STAGES.findIndex(stage => stage.key === editData.current_stage);
  };
        ...
        {editData && (
          <Card sx={{ mb: 3, bgcolor: 'grey.50' }}>
            <CardContent>
              ...
              <Stepper activeStep={getCurrentStageIndex()} alternativeLabel>
